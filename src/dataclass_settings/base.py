...
from dataclass_settings.loader import Loader, LoaderTypes
...
def load_settings(
    ...
    *,
    ...
    loader_types = tuple({context.load_state(loader) for loader in (*loaders, *extra_loaders)})

    result = (
        collect(
            source_cls,
            context=context,
            loaders=loader_types,
            nested_delimiter=nested_delimiter,
        )
        or {}
    )
